...
| `functions/index.js` | Firebase Cloud Functions. `deleteCloudinaryImage` — 기존 Cloudinary 이미지 삭제 전용 (R2 전환 후에도 구 이미지 호환용으로 유지) |
...
## 이미지 업로드 (Cloudflare R2 + Cloudinary 레거시)

### 현재 구조 — Cloudflare R2 (신규)

대역폭 비용 0원. 10GB 무료 저장.

- **R2 버킷**: `fanpick-images` (Cloudflare, 아시아 태평양)
- **R2 공개 URL**: `https://pub-437...r2.dev` (버킷 설정 → 공개 개발 URL)
...
### 레거시 — Cloudinary (기존 게시글 호환용)

- **Cloud name**: `dhkgabcme`
- 신규 업로드는 R2 사용. 기존 Cloudinary 이미지는 그대로 표시됨
- **삭제 분기**: `imageUrl.includes('res.cloudinary.com')` → Firebase Function `deleteCloudinaryImage` 호출
- **Firebase Function**: `functions/index.js` → `deleteCloudinaryImage` (CLOUDINARY_API_KEY/SECRET은 Firebase Secret으로 관리)

### 이미지 삭제 분기 로직 (community.js `deletePost()`)

```javascript
if (imageUrl.includes('res.cloudinary.com')) {
  // 구 Cloudinary 이미지 → Firebase Function 호출
  await firebase.functions().httpsCallable('deleteCloudinaryImage')({ publicId });
} else if (R2_UPLOAD_WORKER_URL) {
  // 신 R2 이미지 → Worker DELETE 호출
  await fetch(`${R2_UPLOAD_WORKER_URL}/${key}`, { method: 'DELETE', headers: { 'X-Firebase-Token': idToken } });
}
```

### 썸네일 변환 (`_getThumbUrl`)

- Cloudinary URL → `w_300,h_300,c_fill,q_60,f_auto` 파라미터 삽입 (~8KB)
- R2/기타 URL → 원본 그대로 반환 (R2는 업로드 시 이미 압축됨)
- 게시글 목록에서만 사용. 상세 페이지는 원본 URL 사용

---

## Firebase Cloud Functions

서버사이드 로직. 소스: `functions/index.js`. Firebase Functions v2 사용.

| 함수 | 역할 | 호출 위치 |
|---|---|---|
| `deleteCloudinaryImage` | 기존 Cloudinary 이미지 서버에서 삭제 | `community.js` `deletePost()` |

- **배포**: `firebase deploy --only functions` (로컬 Firebase CLI 필요)
- **시크릿**: `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` → Firebase Secret Manager에 저장 (코드에 없음)
- **인증**: `request.auth` 필수 (로그인 유저만 호출 가능)
- **신규 이미지(R2)는 Functions 불필요** — R2 Worker가 직접 삭제 처리
