# 팬픽(fanpick) 프로젝트 — Claude 참조 문서

## 프로젝트 개요

K-pop 팬덤 투표 + 커뮤니티 SPA (Single Page Application).  
Firebase Realtime Database + Firebase Auth 기반. 배포: Cloudflare Pages.  
단일 HTML 파일(`docs/index.html`)에 두 탭(투표 / 커뮤니티)이 JS로 전환되는 구조.

**프로덕션 도메인**: `https://fanpick.kr`  
**Cloudflare Pages URL**: `https://fandom-battle.pages.dev` (fanpick.kr의 origin)  
**GitHub 저장소**: `https://github.com/fandomkorea/fandom-battle` (Cloudflare Pages가 자동 배포)

---

## 개념 사전 📖

프로젝트를 이해하는 데 필요한 핵심 개념 정리.

### 캐시 (Cache) — "복사본을 가까이 두기"
자주 쓰는 데이터를 가까운 곳에 미리 저장해두는 것.

- **비유**: 본사 창고(서버)는 멀리 있고, 동네 편의점(캐시)은 가까이 있음. 자주 사는 물건은 편의점에 미리 쌓아둠 → 빠르게 살 수 있음
- **효과**: 서버(Firebase)에 매번 요청하지 않아도 됨 → 속도↑, 비용↓
- **종류**:
  - **브라우저 캐시(localStorage)**: 각자 폰/컴퓨터에 저장. 유저마다 별도 캐시
  - **서버 캐시(CF Worker)**: Cloudflare 서버에 저장. 모든 유저가 공유하는 캐시
- **팬픽 적용**:
  - 랭킹 → CF Worker 60초 공유 캐시 (유저 1만명이 새로고침해도 Firebase 읽기 1번)
  - 커뮤니티 목록 → localStorage 3분 개인 캐시 (3분 내 새로고침해도 Firebase 읽기 0번, 단 새 글 안 보임)
  - 게시글 상세 → sessionStorage 10분 개인 캐시

### CDN (Content Delivery Network) — "전 세계에 복사본 뿌리기"
전 세계 곳곳에 서버를 두고, 유저에게 가장 가까운 서버에서 파일을 서빙하는 시스템.

- **비유**: 유튜브 본 서버가 미국에 있는데 한국 유저가 볼 때마다 미국에서 가져오면 느림. 그래서 한국 서버에도 복사본을 둠 → 빠름
- **팬픽 적용**: Cloudflare Pages가 전 세계 CDN으로 HTML/JS/CSS 서빙. fanpick-rankings Worker도 엣지 서버에 캐시

### .on() vs .once() — "구독 vs 한 번만 읽기"
Firebase Realtime Database에서 데이터를 읽는 두 가지 방식.

```
.on()   = 뉴스 구독   → 데이터 바뀔 때마다 자동으로 앱에 전송 (계속 실시간 수신)
.once() = 뉴스 1회 읽기 → 지금 값 한 번만 읽고 끝
```

- **`.on()` 문제점**: 유저 1만명이 구독 중이면, 데이터 1번 바뀔 때 1만번 전송 → 💸 비용 폭탄
- **팬픽 정책**: 랭킹은 CF Worker가 60초마다 `.once()`로 1번만 읽음. 커뮤니티도 전부 `.once()` 사용
- **⚠️ 절대 금지**: `ranking.js`에 `.on()` 추가 절대 금지

### 쿠키 (Cookie) — "브라우저에 도장 찍어두기"
브라우저에 작은 메모를 저장해두는 것. 주로 로그인 상태 유지에 사용.

- **비유**: 놀이공원 입장 후 손목에 도장 찍어줌. 다시 들어올 때 도장 보여주면 재검사 불필요
- **캐시와 차이**: 캐시는 파일/데이터 임시저장(속도↑), 쿠키는 사용자 정보 저장(로그인 상태 등)
- **팬픽**: Firebase Auth가 토큰을 localStorage에 저장하는 방식 사용 (쿠키 직접 사용 안 함)

### 공유 캐시 vs 개인 캐시 — 비용 차이의 핵심

```
공유 캐시 (CF Worker)          개인 캐시 (localStorage)
──────────────────────         ──────────────────────────
Cloudflare 서버에 저장         각자 폰에 저장
유저 1만명 → Firebase 1번      유저 1만명 → Firebase 최대 1만번
유저 수 무관하게 고정 비용      유저 수에 비례해서 비용 증가
```

→ 랭킹은 공유 캐시(CF Worker), 커뮤니티는 개인 캐시(localStorage)

---

## 인프라 / 서비스 현황 ⚙️

> 어떤 서비스를 어디서 쓰는지 한눈에 보는 표. URL/이름 변경 시 반드시 여기도 업데이트.

### 호스팅

| 서비스 | 용도 | URL / 이름 |
|---|---|---|
| **Cloudflare Pages** | 사이트 호스팅 (정적 파일 서빙) | `fanpick` 프로젝트 → `fanpick.kr` |
| **GitHub** | 소스 코드 저장소 (push하면 Cloudflare Pages 자동 배포) | `fandomkorea/fandom-battle` |

### Cloudflare Workers

| Worker 이름 | 용도 | URL |
|---|---|---|
| **fanpick-rankings** | Firebase rankings를 60초 엣지 캐시로 서빙 (랭킹 실시간 업데이트) | `https://fanpick-rankings.coder-leebeegle2.workers.dev` |
| **fanpick-upload** | R2 이미지 업로드/삭제 (Firebase 토큰 인증 후 R2 PUT/DELETE) | `https://fanpick-upload.coder-leebeegle2.workers.dev` |

- **소스**: `cloudflare-worker.js` (fanpick-rankings), `r2-upload-worker.js` (fanpick-upload)
- **ALLOWED_ORIGINS** (fanpick-upload): `fanpick.kr`, `www.fanpick.kr`, `fandom-battle.pages.dev`, `localhost`

### Cloudflare R2

| 항목 | 값 |
|---|---|
| 버킷 이름 | `fanpick-images` |
| 리전 | 아시아 태평양 |
| 공개 URL | `fanpick-upload` Worker의 `PUBLIC_R2_URL` 환경변수에 저장 |
| 용도 | 커뮤니티 게시글 이미지 저장 (대역폭 비용 0원) |

### Firebase (프로젝트: `fanpick` / ID: `fandom-battle-92aa8`)

| 항목 | 값 / 설명 |
|---|---|
| **Realtime Database** | `fandom-battle-92aa8-default-rtdb.firebaseio.com` — 투표·유저·커뮤니티 데이터 |
| **Authentication** | Google / Twitter / Apple 로그인. 승인된 도메인: `fanpick.kr` |
| **App Check** | reCAPTCHA v3 Enforce 적용 — 봇 차단. REST API는 DB Secret으로 우회 |
| **Cloud Functions** | `deleteCloudinaryImage` — 구 Cloudinary 이미지 삭제 전용 (레거시 호환) |
| **DB Secret** | Cloudflare Workers 환경변수 `FIREBASE_DB_SECRET`에 저장 (App Check 우회용) |

### Google reCAPTCHA v3

| 항목 | 값 |
|---|---|
| 사이트 키 | `6LdyUgItAAAAAKa2BT84IgauQYdHGDIqTgU0XIUK` (공개값) |
| 등록 도메인 | `fanpick.kr` |
| 용도 | Firebase App Check Provider |

### 도메인

| 도메인 | 구매처 | DNS 관리 | 용도 |
|---|---|---|---|
| `fanpick.kr` | Gabia | Cloudflare (네임서버: eloise/merlin.ns.cloudflare.com) | 메인 도메인 |

### GitHub Actions

| 워크플로우 | 역할 | 주기 |
|---|---|---|
| `update-rankings.yml` | Firebase rankings → `docs/data/rankings.json` 커밋 (CF Worker 장애 시 폴백용) | 1시간마다 |
...
| `r2-upload-worker.js` | Cloudflare R2 이미지 업로드/삭제 Worker 소스 (저장소 루트). Firebase 토큰 검증 후 R2 PUT/DELETE |
| `functions/index.js` | Firebase Cloud Functions. 현재 배포된 함수 없음 (Cloudinary 레거시 함수 제거됨) |
...
- push하면 Cloudflare Pages가 자동으로 `docs/` 폴더를 빌드·배포함
...
- **CF Worker URL**: `https://fanpick-rankings.coder-leebeegle2.workers.dev`
- **상수 위치**: `ranking.js` 최상단 `const CF_RANKINGS_WORKER_URL`
- GitHub Actions 워크플로우: `.github/workflows/update-rankings.yml` (1시간마다 폴백용 JSON 갱신)
...
- **SW 파일**: `docs/sw.js` (Cloudflare Pages / 커스텀 도메인 양쪽에서 동일하게 동작)
- **캐시 전략**:
...
대역폭 비용 0원. 10GB 무료 저장.

- **R2 버킷**: `fanpick-images` (Cloudflare, 아시아 태평양)
- **업로드 Worker**: `fanpick-upload` → `https://fanpick-upload.coder-leebeegle2.workers.dev`
- **Worker 소스**: `r2-upload-worker.js` (저장소 루트)
- **상수 위치**: `community.js` 최상단 `const R2_UPLOAD_WORKER_URL`
- **업로드 흐름**: 이미지 선택 → Canvas 압축(WebP, max 1200px, q=0.82) → Firebase ID 토큰 발급 → Worker PUT `/{uid}/{timestamp}.webp` → R2 저장 → 공개 URL 반환
- **DB 저장**: `community/{fandom}/{postId}/imageUrl` 에 R2 공개 URL 저장
- **보안**: Worker가 Firebase 토큰 검증, key에 본인 uid 포함 여부 확인

### 이미지 삭제 로직 (community.js `deletePost()`)

```javascript
if (postData.imagePublicId && R2_UPLOAD_WORKER_URL) {
...
- R2는 업로드 시 이미 WebP 압축됨 → 원본 URL 그대로 반환

---

## Firebase Cloud Functions

현재 배포된 함수 없음. `functions/index.js`는 빈 파일로 유지.  
이미지 삭제는 `fanpick-upload` Cloudflare Worker가 직접 처리.
