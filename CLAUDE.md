...
| `docs/js/ranking.js` | 랭킹 CDN 폴링 (`data/rankings.json` 60초 폴링), `renderRankings()`, `localVoteUpdate()`, 트렌딩 감지 |
...
| `docs/data/rankings.json` | 랭킹 정적 JSON. GitHub Actions가 5분마다 자동 갱신. 클라이언트가 GitHub Pages CDN에서 읽음 |
...
| `js/voting.js` | v=20260609a |
| `js/ranking.js` | v=20260609a |
| `js/utils.js` | v=20260529e |
| `js/community.js` | v=20260609b |
...
**구조**: GitHub Actions → Firebase REST API 읽기 (5분마다 1회) → `docs/data/rankings.json` 커밋 → GitHub Pages CDN 서빙 → 클라이언트 60초 폴링

- GitHub Actions 워크플로우: `.github/workflows/update-rankings.yml`
- Firebase reads: **288회/일** (사용자 수와 무관하게 고정)
- `rankings` 경로는 `.read: true` (공개) → Actions에 인증 키 불필요
- CDN 실패 시: Firebase REST API 직접 읽기 자동 폴백
...
|---|---|---|
| `rankings` | **CDN 폴링만** | `.on()` / `.once()` 모두 금지 |
...
→ `ranking.js`에 `.on()` 추가 금지 — CDN 폴링 아키텍처 유지
