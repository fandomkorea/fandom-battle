...
function refreshCommunityPosts() {
  if (currentSelectedTab === 'all') {
    showToast("🔄 새로고침 중...");
    if (currentAllSubTab === 'overview')       { loadCategoryOverview(true); }
    else if (currentAllSubTab === 'best')      { loadBestPosts(true); }
    else if (currentAllSubTab === 'grouped')   { _loadGroupedFeed(true); }
    else                                       { loadCategoryPosts(currentAllSubTab, true); }
    return;
  }
  ...
  loadFandomCategoryOverview(selectedFandom);
}
...
// ── 전체 피드 서브탭 상태 ──
// 'overview' = 카테고리별 그룹 미리보기(기본)
// 'best' | 'humor' | 'daily' | 'music' | 'chat' = 해당 카테고리 전체 목록
// 'grouped' = 팬덤별 그룹뷰 (레거시)
let currentAllSubTab = 'overview';

// ── 팬덤별 카테고리 개요 캐시 ──
let _fandomOverviewCache = {}; // { fandom: { posts, loadedAt } }
const _FANDOM_OVERVIEW_TTL = 3 * 60 * 1000; // 3분

// ── 글쓰기 카테고리 선택 ──
let selectedPostCategory = 'general';

// ── 서브탭별 캐시 ──
let _bestPostsCache = null;
let _bestPostsCacheAt = 0;
let _categoryPostsCache = {}; // { category: { posts, loadedAt } }
let _overviewCache = null;
let _overviewCacheAt = 0;
const _BEST_POSTS_CACHE_TTL    = 5 * 60 * 1000; // 5분
const _CATEGORY_POSTS_CACHE_TTL = 3 * 60 * 1000; // 3분
const _OVERVIEW_CACHE_TTL      = 5 * 60 * 1000; // 5분
...
function _invalidatePostListCache(fandom) {
  ...
  // 팬덤 개요 캐시도 무효화
  if (fandom && _fandomOverviewCache[fandom]) delete _fandomOverviewCache[fandom];
}
...
async function submitPost() {
  ...
  try {
    ...
    const postData = {
      ...
      category: selectedPostCategory || 'general',
...
function selectFandomTab(tabId) {
  ...
  if (tabId !== 'all') {
    ...
    const subTabBar = document.getElementById("allFeedSubTabBar");
  ...
  if (tabId === 'all') {
    ...
    loadFandomCategoryOverview(tabId); // ★ 카테고리 섹션 뷰
  }
...
async function loadAllFandomPosts(forceRefresh = false) {
  ...
  // 현재 서브탭에 맞는 로더 호출
  if (currentAllSubTab === 'overview') {
    await loadCategoryOverview(forceRefresh);
  } else if (currentAllSubTab === 'best') {
    await loadBestPosts(forceRefresh);
  } else if (currentAllSubTab === 'grouped') {
    await _loadGroupedFeed(forceRefresh);
  } else {
    await loadCategoryPosts(currentAllSubTab, forceRefresh);
  }
}

// ── 팬덤별 그룹 피드 로드 (서브탭 'grouped') ──
async function _loadGroupedFeed(forceRefresh = false) {
  const postsList = document.getElementById("communityPostsList");
  ...
  postsList.innerHTML = `
    <div class="community-empty">
      <div class="spinner" style="display:inline-block;margin-bottom:12px"></div>
      <div class="community-empty-text">팬덤별 피드를 불러오는 중...</div>
    </div>
  `;
...
// ── 랭킹 상위 10개 팬덤 가져오기 (베스트/카테고리용) ──
function _getTop10Fandoms() {
  if (allRankingsData && Object.keys(allRankingsData).length > 0) {
    return Object.entries(allRankingsData)
      .filter(([g]) => ALL_GROUPS.includes(g))
      .sort((a, b) => b[1] - a[1])
  ...
  return ALL_GROUPS.slice(0, 10);
}

// ── 전체 피드 서브탭 바 렌더링 ──
function _renderAllSubTabBar() {
...
async function switchAllSubTab(tabId, fromPopstate = false) {
  ...
  else                          { await loadCategoryPosts(tabId, false); }
}

// ── 베스트 글 로드 (좋아요+댓글 가중치, 7일 이내) ──
async function loadBestPosts(forceRefresh = false) {
  const postsList = document.getElementById("communityPostsList");

  if (!forceRefresh && _bestPostsCache && Date.now() - _bestPostsCacheAt < _BEST_POSTS_CACHE_TTL) {
    _renderBestPosts(_bestPostsCache);
    communityPostsLoaded = true;
    return;
  }
  ...
  postsList.innerHTML = `
    <div class="community-empty">
      <div class="spinner" style="display:inline-block;margin-bottom:12px"></div>
      <div class="community-empty-text">베스트 글을 불러오는 중...</div>
    </div>
  `;

  const top10 = _getTop10Fandoms();
  ...
  try {
    ...
    allPosts.sort((a, b) => {
      ...
      return sB - sA || (b.post.timestamp || 0) - (a.post.timestamp || 0);
    });

    const best = allPosts.slice(0, 20);
    _bestPostsCache = best;
    _bestPostsCacheAt = Date.now();
    communityPostsLoaded = true;
    _renderBestPosts(best);
  } catch (e) {
    console.error("베스트 글 로드 실패:", e);
    postsList.innerHTML = `<div class="community-empty"><div class="community-empty-icon">⚠️</div><div class="community-empty-text">불러오지 못했어요<br>잠시 후 다시 시도해주세요</div></div>`;
  }
}

// ── 카테고리별 글 로드 ──
async function loadCategoryPosts(category, forceRefresh = false) {
  const postsList = document.getElementById("communityPostsList");
  const catInfo = { humor:'유머', daily:'일상', music:'음악/무대', chat:'잡담' };

  const cache = _categoryPostsCache[category];
  if (!forceRefresh && cache && Date.now() - cache.loadedAt < _CATEGORY_POSTS_CACHE_TTL) {
    _renderCategoryPosts(cache.posts, category);
    communityPostsLoaded = true;
    return;
  }
  ...
  postsList.innerHTML = `
    <div class="community-empty">
      <div class="spinner" style="display:inline-block;margin-bottom:12px"></div>
      <div class="community-empty-text">${catInfo[category] || category} 게시글을 불러오는 중...</div>
    </div>
  `;
  ...
  try {
    ...
    top10.forEach((fandom, i) => {
      const val = snapshots[i].val() || {};
      Object.entries(val).forEach(([postId, post]) => {
        if (!post.isHidden && post.category === category) {
          allPosts.push({ fandom, postId, post });
        }
      });
    });
    ...
    _categoryPostsCache[category] = { posts, loadedAt: Date.now() };
    communityPostsLoaded = true;
    _renderCategoryPosts(posts, category);
  } catch (e) {
...
function _renderBestPosts(posts) {
  ...
  if (!posts || posts.length === 0) {
    postsList.innerHTML = `
      <div class="community-empty">
        <div class="community-empty-icon">🏆</div>
        <div class="community-empty-text">아직 베스트 글이 없어요<br><span style="font-size:0.82rem;opacity:0.7">좋아요를 많이 받은 글이 여기 등장해요!</span></div>
      </div>
    `;
    return;
  }
...
function _renderCategoryPosts(posts, category) {
  ...
  const catInfo = {
    humor: { icon:'😂', label:'유머' },
    daily: { icon:'🌸', label:'일상' },
    music: { icon:'🎵', label:'음악/무대' },
    chat:  { icon:'💬', label:'잡담' },
  };
  const info = catInfo[category] || { icon:'📋', label:category };
  if (!posts || posts.length === 0) {
    postsList.innerHTML = `
      <div class="community-empty">
        <div class="community-empty-icon">${info.icon}</div>
        <div class="community-empty-text">${info.label} 게시글이 없어요<br><span style="font-size:0.82rem;opacity:0.7">글 쓸 때 '${info.label}' 카테고리를 선택하면 여기 등장해요!</span></div>
      </div>
    `;
    return;
  }
...
// 상위 10개 팬덤 데이터를 한 번에 받아 베스트·유머·일상·음악·잡담 섹션으로 분류
async function loadCategoryOverview(forceRefresh = false) {
  const postsList = document.getElementById("communityPostsList");

  if (!forceRefresh && _overviewCache && Date.now() - _overviewCacheAt < _OVERVIEW_CACHE_TTL) {
    _renderCategoryOverview(_overviewCache);
    communityPostsLoaded = true;
    return;
  }
  ...
  try {
    // 팬덤당 최신 50개 — 한 번 로드해서 모든 카테고리에 재활용
    const snapshots = await Promise.all(
      top10.map(fandom =>
        db.ref(`community/${fandom}`)
          .orderByChild('timestamp')
    ...
    top10.forEach((fandom, i) => {
      const val = snapshots[i].val() || {};
      Object.entries(val).forEach(([postId, post]) => {
        if (!post.isHidden) allPosts.push({ fandom, postId, post });
      });
    });
    ...
    // 카테고리별: category 필드 기준, 최신순 Top 5
    const byCat = (cat) => allPosts
      .filter(p => p.post.category === cat)
      .sort((a, b) => (b.post.timestamp || 0) - (a.post.timestamp || 0))
      .slice(0, 5);

    const sections = [
      { id: 'best',  label: '🏆 베스트',    posts: bestPosts },
      { id: 'humor', label: '😂 유머',      posts: byCat('humor') },
      { id: 'daily', label: '🌸 일상',      posts: byCat('daily') },
      { id: 'music', label: '🎵 음악/무대', posts: byCat('music') },
      { id: 'chat',  label: '💬 잡담',      posts: byCat('chat') },
    ];

    _overviewCache = sections;
    _overviewCacheAt = Date.now();
    communityPostsLoaded = true;
    _renderCategoryOverview(sections);
  } catch (e) {
    console.error("카테고리 개요 로드 실패:", e);
    postsList.innerHTML = `
      <div class="community-empty">
        <div class="community-empty-icon">⚠️</div>
        <div class="community-empty-text">피드를 불러오지 못했어요<br>잠시 후 다시 시도해주세요</div>
    ...
    return `
      <div class="fandom-group-section">
        <div class="fandom-group-header">
          <span class="fandom-group-title">${label}</span>
          <button class="fandom-group-more" onclick="switchAllSubTab('${id}')">더보기 ›</button>
        </div>
...
// ── 팬덤 카테고리 개요 로드 ──
async function loadFandomCategoryOverview(fandom, forceRefresh = false) {
  const postsList = document.getElementById("communityPostsList");
  const sortGroup = document.getElementById("sortButtonGroup");
  if (sortGroup) sortGroup.style.display = 'none';
  ...
  if (!forceRefresh && cache && Date.now() - cache.loadedAt < _FANDOM_OVERVIEW_TTL) {
    _renderFandomCategoryOverview(fandom, cache.posts);
    communityPostsLoaded = true;
    return;
  }

  // localStorage 캐시 활용 (기존 캐시 재사용)
  if (!forceRefresh) {
    const lsRaw = _getCachedPostList(fandom);
    if (lsRaw && Object.keys(lsRaw).length > 0) {
      const posts = Object.entries(lsRaw)
        .filter(([, p]) => !p.isHidden)
        .map(([postId, post]) => ({ postId, post }));
      _fandomOverviewCache[fandom] = { posts, loadedAt: Date.now() };
      _renderFandomCategoryOverview(fandom, posts);
      communityPostsLoaded = true;
      return;
    }
  }
  ...
  postsList.innerHTML = `
    <div class="community-empty">
      <div class="spinner" style="display:inline-block;margin-bottom:12px"></div>
      <div class="community-empty-text">불러오는 중...</div>
    </div>
  `;

  try {
    const snap = await db.ref(`community/${fandom}`)
      .orderByChild('timestamp')
      .limitToLast(50)
      .once("value");

    const all = snap.val() || {};
    _setCachedPostList(fandom, all);
    ...
    console.error("팬덤 개요 로드 실패:", e);
    postsList.innerHTML = `
      <div class="community-empty">
        <div class="community-empty-icon">⚠️</div>
        <div class="community-empty-text">불러오지 못했어요<br>잠시 후 다시 시도해주세요</div>
      </div>
    `;
  }
}
...
// ── 팬덤 카테고리 개요 렌더링 ──
function _renderFandomCategoryOverview(fandom, allPosts) {
  const postsList = document.getElementById("communityPostsList");
  ...
  const sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const sorted = [...allPosts].sort((a, b) => (b.post.timestamp || 0) - (a.post.timestamp || 0));
  ...
  const byCat = (cat) => sorted.filter(p => p.post.category === cat).slice(0, 5);

  const catEmptyMsg = {
    latest: '아직 게시물이 없어요',
    best:   '아직 인기 글이 없어요 (좋아요가 쌓이면 등장해요!)',
    humor:  '유머 글이 없어요 — 글 쓸 때 😂 유머를 선택해보세요!',
    daily:  '일상 글이 없어요 — 글 쓸 때 🌸 일상을 선택해보세요!',
    music:  '음악/무대 글이 없어요 — 글 쓸 때 🎵 음악을 선택해보세요!',
    chat:   '잡담 글이 없어요 — 글 쓸 때 💬 잡담을 선택해보세요!',
  ...
  const sections = [
    { id: 'latest', label: '📋 최신',      posts: latest },
    { id: 'best',   label: '🏆 베스트',    posts: best },
    { id: 'humor',  label: '😂 유머',      posts: byCat('humor') },
    { id: 'daily',  label: '🌸 일상',      posts: byCat('daily') },
    { id: 'music',  label: '🎵 음악/무대', posts: byCat('music') },
    { id: 'chat',   label: '💬 잡담',      posts: byCat('chat') },
  ...
  postsList.innerHTML = sections.map(({ id, label, posts }) => {
    ...
    return `
      <div class="fandom-group-section">
        <div class="fandom-group-header">
          <span class="fandom-group-title">${label}</span>
          <button class="fandom-group-more" onclick="showFandomCategoryFull('${escAttr(fandom)}','${id}')">더보기 ›</button>
        </div>
        <div class="fandom-group-posts">${postsHtml}</div>
      </div>
    `;
...
// ── 팬덤 카테고리 전체 목록 (더보기 클릭 시) ──
function showFandomCategoryFull(fandom, catId) {
  const sortGroup = document.getElementById("sortButtonGroup");
  if (sortGroup) sortGroup.style.display = 'none';
  ...
  const catLabels = {
    latest: '최신', best: '베스트',
    humor: '유머', daily: '일상', music: '음악/무대', chat: '잡담',
  };
  ...
  if (catId === 'latest') {
    ...
    filtered = [...allPosts]
      .filter(p => p.post.category === catId)
      .sort((a, b) => (b.post.timestamp || 0) - (a.post.timestamp || 0));
  }
  ...
  backBtn.style.cssText = 'display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:10px;color:var(--text);font-size:0.85rem;font-weight:600;padding:8px 14px;cursor:pointer;margin-bottom:14px;font-family:inherit;width:100%';
  backBtn.innerHTML = `← ${escHtml(fandom)} 홈으로`;
  backBtn.onclick = () => loadFandomCategoryOverview(fandom);

  const subTitle = document.createElement('div');
  subTitle.style.cssText = 'font-size:0.82rem;font-weight:700;color:var(--muted);margin-bottom:10px';
  subTitle.textContent = `${catLabels[catId] || catId} 전체`;

  postsList.innerHTML = '';
  postsList.appendChild(backBtn);
  postsList.appendChild(subTitle);

  if (!filtered || filtered.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'community-empty';
    empty.innerHTML = `<div class="community-empty-icon">📭</div><div class="community-empty-text">게시물이 없어요</div>`;
    postsList.appendChild(empty);
    return;
  }

  filtered.forEach(({ postId, post }, idx) => {
    const postEl = renderPost(fandom, postId, post, idx, false);
    postsList.appendChild(postEl);
  });
}
